<template>
  <div style="padding-left: 5px;padding-right: 10px;">
    ...
    <div id="people" class="dataTables_wrapper no-footer my-5">
      <v-client-table :data="orderListUser" :columns="columns" :options="options" class="text-center">
        ...
        <div slot="status" slot-scope="props">
          ...
          <a-tag v-if="props.row.status == 2" color="#108ee9" style="background-color: #f5222d;">CANCELED</a-tag>
        </div>
        <div slot="action" slot-scope="props">
          <a-button-group>
           <a-button @click="modal(props.row.id)" icon="eye"/>
          </a-button-group>
        </div>
      </v-client-table>
    </div>
...
export default {
  ...
  created() {
    if (!this.orderListUser.length > 0) this.$store.dispatch('ORDER_LIST_USER');
  },
  methods: {
    modal(e) {
    ...
    confirm(id) {
      ApiService.delete('orders/' + id)
          .then(() => {
            this.$store.commit('ORDER_REMOVE_USER', id);
  ...
  computed: {
    ...mapGetters(["isBangla", "orderListUser"]),
    columns() {
